...
contract SIGILS is ERC721Royalty, Ownable(msg.sender) {
    ...
    string private htmlEnd = '</script></body></html>';

    /// @notice Mints tokens to an address
    /// @param _to The address to mint the token to
    function MINT(address _to) external onlyOwner {
        sigilAddress[_nextTokenId] = _to;
    ...
    function getHTML(
        ...
        string memory html = string(
            abi.encodePacked(
                htmlStart,
                'let address = "',
                Strings.toHexString(uint160(sigilAddress[_tokenId]), 20),
                '";',
                script,
                htmlEnd
            )
    ...
    function tokenURI(
        ...
        string memory uri = string(
            abi.encodePacked(
                ...
                '", "animation_url": "',
                base64HTML,
                '", "attributes": [{"trait_type": "Address", "value": "',
                Strings.toHexString(uint160(sigilAddress[_tokenId]), 20),
                '"}]}'
            )
        );
    ...
    /// @notice Allows owner to set the SVG
    /// @param _svg The new SVG to be set
    function setSVG(string calldata _svg) external onlyOwner {
        svg = _svg;
    }

    /// @notice Allows owner to set the HTML start
    /// @param _htmlStart The new HTML start to be set
    function setHTMLStart(string calldata _htmlStart) external onlyOwner {
        htmlStart = _htmlStart;
    }

    /// @notice Allows owner to set the HTML end
    /// @param _htmlEnd The new HTML end to be set
    function setHTMLEnd(string calldata _htmlEnd) external onlyOwner {
        htmlEnd = _htmlEnd;
    }
